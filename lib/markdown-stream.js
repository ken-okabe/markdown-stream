...
module.exports = {
  activate: function(state)
  {
    ...
    {
      //set marked-------
      marked.setOptions(atom.config.get('markdown-stream.markedSettings'));
      marked.setOptions(
      {
              ...
              c
                .pipe(d(
                {
                  ...
                  bind: function(val, f) // must keep this format
                  {
                    ...
                    {
                      console.log('obj.html changed or triggered');
    ...
    atom
      ...
      .eachEditorView(function(editorView)
      {
        ...
        editor
          .on('contents-modified', function()
          {
            process.nextTick(function()
            {
              ...
              compileMarkdown(editor.getText());
        ...
        editorView
          .on('focusin', function()
          {
            console.log('editorView:focusin ');
            compileMarkdown(editor.getText());
    ...
    var compileMarkdown = function(txt)
    {

      console.log('newlinesAsIsMode: ' + atom.config.get('markdown-stream.newlinesAsIsMode'));
      if (atom.config.get('markdown-stream.newlinesAsIsMode'))
      {
        obj.html = '<style type="text/css">p {margin: 0em}</style>' +
          marked(hack(txt));
      }
      else
      {
